...
/*   Updated: 2025/09/18 10:41:00 by nacuna-g         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void update_env_var(char ***envp, char *var, char *value)
{
    ...
    // Buscar variable existente (mismo código que antes)
    i = 0;
    while ((*envp)[i] != NULL)
    {
    ...
    // Crear nuevo array con calloc
    char **new_envp = calloc(env_size + 2, sizeof(char *));  // Inicializa todo a NULL
    if (new_envp == NULL)
    {
        perror("calloc envp");
        return;
    ...
    new_envp[i] = new_var;
    // new_envp[i + 1] ya es NULL por calloc
    
    // Liberar array y strings viejos
    i = 0;
    while ((*envp)[i] != NULL)
    {
        free((*envp)[i]);
...
static int builtin_cd(char **args, char ***envp)
{
    ...
    {
        // Fallback mejorado: construir path manualmente si relative
        if (target[0] == '/')
